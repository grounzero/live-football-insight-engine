...
> short horizon, never statements of fact. This is not an injury, betting, officiating or
> player-safety system.

**Live demo:** _(not yet deployed — see [Public demo deployment](#public-demo-deployment); add the
generated URL here and in the GitHub **About** panel once it is up)_. The hosted demo replays a
**generated synthetic fixture**, not real match data, and scores it with a model trained on that
fixture. The results below were measured on Metrica matches and describe a different artifact.
...
The container is self-contained: it builds the React app, carries a small model trained on
generated data, and starts with no dataset, no bind mount and no network access. A bare
`docker run -p 8000:8000 <image>` is the hosted demo.

**What the hosted demo is, precisely.** Both facts are also shown in the page footer and returned
by `GET /ready`, so the page and the service cannot disagree:

- the replay is a **deterministic synthetic fixture** generated by
  [`data/synthetic.py`](src/football_insights/data/synthetic.py) — not a real match, and not the
  Metrica sample data, which carries no redistribution licence and is never baked into an image;
- the predictor is **`demo-synthetic-gru`**, a small GRU trained during the image build on that
  same generated data and exported to ONNX. It demonstrates the path — features, sequence model,
  export, live scoring — and **is not the model the [Results](#results) section reports**. Those
  numbers were measured on Metrica matches and say nothing about this artifact. If the artifact is
  missing the service still starts on the rule-based fallback and labels itself `Fallback — not
  ML` in the page and `is_ml: false` in the API.

### Architecture

One container, one process, one replica. The replay, its subscriber set and the Prometheus
registry are per-process, so a second worker would serve a second, divergent replay from the same
URL. Public mode is therefore read-only: `POST /replay/control` and `POST /replay/match` are not
registered at all, so no visitor can pause, rewind or re-point the replay everyone else is
watching. The replay starts when the first viewer connects, loops continuously, and pauses itself
when the last viewer leaves so an unwatched container costs nothing.

### Environment variables

Every one of these is already baked into the image. Nothing needs setting to deploy.

| Variable | Image default | Why |
|---|---|---|
| `PORT` | `8000` | Injected by the platform; `serve` reads it above `--port`'s fallback and above `FI_SERVICE__*`. Invalid or out-of-range values fail at startup with a readable message. |
| `FI_SERVICE__HOST` | `0.0.0.0` | The loopback default is right locally and unreachable in a container. |
| `FI_SERVICE__PUBLIC_DEMO` | `1` | Generated fixture, read-only replay, single-match catalogue. |
| `FI_REPLAY__LOOP` | `1` | Restart at kick-off instead of ending. |
| `FI_MODEL__MODEL_NAME` | `demo-synthetic-gru` | Load the demo artifact rather than searching for a reference model that is not there. |
| `FI_PATHS__REGISTRY_DIR` | `/opt/artifacts/registry` | Where the build put it. |

No volume, no dataset download, no secrets, no model credentials.

### Verify it locally first

```bash
make container            # docker build, then the full smoke test
# or, step by step:
docker build -t football-insights:deployment-test .
make container-smoke IMAGE=football-insights:deployment-test
```

The smoke test starts the image on a non-default port with no mounts and checks `/health`,
`/ready`, the page, a hashed asset, a live SSE event, the Docker health probe, that exactly one
worker is running, and that the final image contains neither Node nor PyTorch. It prints container
logs on any failure. CI runs the same script against the same image.

### Deploy to Railway

1. Push this repository to GitHub.
2. In Railway: **New Project → Deploy from GitHub repo**, and pick it.
3. Railway reads [`railway.toml`](railway.toml) and builds with the Dockerfile. No build command,
   start command or environment variable needs setting — the image carries its own defaults.
4. **Settings → Networking → Generate Domain.** Railway assigns a public URL and injects `PORT`.
5. Wait for the health check at `/ready` to pass. Until it does, the deployment is not routed.

The first build takes several minutes: it installs CPU PyTorch in a build stage, trains the demo
model and asserts ONNX parity before the image is assembled. Later builds reuse cached layers, and
a frontend-only change does not retrain the model.

Health and readiness on the deployed URL:

```
https://<your-domain>/health   # liveness; 200 while the process is up
https://<your-domain>/ready    # readiness; mode, data source, predictor, UI
```

`/ready` gates traffic because it answers 503 until the fixture is playable, a predictor is loaded
and the built page is present. Docker's own `HEALTHCHECK` stays on `/health`, which is liveness and
must not depend on any of that. Neither response contains a path, a credential or a config
fingerprint.

### Known limitation

The demo is stateful in one process. Every viewer watches the same replay at the same position,
and it cannot be scaled horizontally without redesigning that shared state. That is why the
controls are read-only rather than merely disclosed, and why `numReplicas = 1` is pinned rather
than defaulted.
...
| `docker build` and the container smoke test | run locally. The image builds from a clean context, starts with no mounts and no dataset on an injected `PORT=8087`, and serves `/health`, `/ready`, the page, a hashed asset and a live SSE event. `make container` reproduces it; CI runs the same script. |
| Railway deployment | **not performed.** `railway.toml` and the steps below are written against Railway's documented Dockerfile builder and are unverified against a live account. |
...
- **The hosted demo measures nothing.** It replays generated data through a model trained on
  generated data. It shows that the pipeline works end to end; it is not evidence about football.
- **The hosted demo is single-process and stateful.** One replay, shared by every viewer, and not
  horizontally scalable as built.
